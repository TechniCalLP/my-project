...
export function MobileHeader({ user, className, logoUrl }: MobileHeaderProps) {
  return (
    <header className={`bg-white border-b sticky top-0 z-40 ${className ?? ""}`}>
      <div className="flex items-center justify-between h-14 px-4">
        <Link href="/dashboard" className="flex items-center gap-2">
          <div className="relative w-8 h-8">
            <CollegeLogoImage logoUrl={logoUrl} width={32} height={32} className="rounded-lg object-cover" />
          </div>
